...
```{r}
data <- read.csv("/home/RHHLab/Research/Colab/Turag/Python/DL Lab/Multivariate/DOA/epochs100/metrics.csv")
data
```
...
```
```{r}
dso <- c("General", "MA", "HY", "AE", "MA AE", "MA HY")

data$ModelCat <- factor(data$ModelCat, levels = dso)
data
```
...
ggplot(data, aes(x = Model, y = y))+
  geom_bar(
    aes(fill = ModelCat), stat = "identity", color = "white",
    position = position_dodge(0.9)
    )+
  facet_wrap(~Metrics) + 
  fill_palette("jco")+
